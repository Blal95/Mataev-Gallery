...
describe("api", () => {
  ...
  it("nextOffset equals offset+limit when page is full", async () => {
    ...
    const res = await buildPhotosResponse(db, "https://cdn.x", { limit: 2, offset: 0 })
  ...
  it("nextOffset is null when page is partial (last page)", async () => {
    ...
    await insertPhoto(db, row("p1"), [])
    await insertPhoto(db, row("p2"), [])
    const res = await buildPhotosResponse(db, "https://cdn.x", { limit: 5, offset: 0 })
    expect(res.photos).toHaveLength(2)
    expect(res.nextOffset).toBeNull()
  })
  it("nextOffset correctly advances on page 2", async () => {
    ...
    const p1 = await buildPhotosResponse(db, "https://cdn.x", { limit: 2, offset: 0 })
    expect(p1.nextOffset).toBe(2)
    const p2 = await buildPhotosResponse(db, "https://cdn.x", { limit: 2, offset: p1.nextOffset! })
    expect(p2.nextOffset).toBe(4)
    const p3 = await buildPhotosResponse(db, "https://cdn.x", { limit: 2, offset: p2.nextOffset! })
  ...
  it("tags are always full regardless of pagination", async () => {
    ...
    const res = await buildPhotosResponse(db, "https://cdn.x", { limit: 1, offset: 0 })
